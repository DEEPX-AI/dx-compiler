...
| 요구사항 | 상세 |
|---|---|
| **DX-COM** | v2.2.1 이상 설치 (`pip install dx-com` 또는 DEEPX 패키지 저장소) |
...
| 스킬 | 설명 |
|------|------|
| `/dx-agentic-compiler-compile` | ONNX → DXNN 컴파일 단계별 워크플로우 |
| `/dx-agentic-compiler-convert` | PyTorch → ONNX 변환 단계별 워크플로우 |
| `/dx-agentic-compiler-validate` | 컴파일된 .dxnn 모델 출력 검증 |
...
| **OpenCode** | CLI | `AGENTS.md` + `opencode.json` | `@dx-compiler-builder "프롬프트"` 또는 `/dx-agentic-compiler-compile` |
...
| **OpenCode** | `/dx-agentic-compiler-convert` 또는 `@dx-model-converter` 뒤에 프롬프트를 입력합니다. |
...
| **Cursor** | `dx-compiler/`를 열고 프롬프트를 입력합니다. |
| **OpenCode** | `/dx-agentic-compiler-compile` 또는 `@dx-dxnn-compiler` 뒤에 프롬프트를 입력합니다. |
...
모든 컴파일 산출물은 기본적으로 `dx-agentic-dev/<session_id>/`에 저장됩니다.
각 컴파일 세션이 자체 완결적이고 재현 가능하도록 합니다.

**세션 ID 형식**: `YYYYMMDD-HHMMSS_<model>_<task>`

| 출력 유형 | 경로 | 조건 |
|---|---|---|
| **기본 (격리)** | `dx-agentic-dev/<session_id>/` | 사용자가 별도 지정하지 않으면 항상 |
