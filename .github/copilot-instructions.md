...
# Step 2: Export to ONNX (using skill /dx-convert-model)
# Step 3: Compile to DXNN (using skill /dx-compile-model)
# Step 4: Validate (using skill /dx-validate-compile)
```
...
| `/dx-brainstorm-and-plan` | Brainstorm and plan before any compilation task |
| `/dx-tdd` | Test-driven development — validate each step incrementally |
